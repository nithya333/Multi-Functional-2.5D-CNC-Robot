...
def gcode_sender_func(filename, baudrate, port):
    ...
    for code in tqdm4codes:
        ...
        if code.strip().startswith(';') or code.isspace() or len(code) <=0:
            ...
            # if code.strip().startswith('G0') or code.strip().startswith('G1'):
            #     X_req = (float)(re.search(patternX, code).group(1))
            #     Y_req = (float)(re.search(patternY, code).group(1))
            s.write((code+'\n').encode())
            while(1): # Wait untile the former gcode has been completed.
                ...
                if reply.startswith(b'ok'):
                    ...
                    while (running):
                        if s.in_waiting:
                            print(f"Discarding .... {s.readline()}")
                        s.write(b'?')
                        # time.sleep(0.01)
                        reply = s.readline()
                        running = False if reply.startswith(b'<Idle') else True
                        # if not running:
                        #     break
                        # matched_grps = re.search(pat, reply.decode('ascii'))
                        # X_curr = (float) (matched_grps.group(1))
                        # Y_curr = (float) (matched_grps.group(2))
                        # print(f"##### {reply}  {X_req - X_curr}   {Y_req - Y_curr}######")
                        # print(f"##### {reply} ######")
                    # print(s.in_waiting)

                    # for i in range(1,6):
                    #     print(f"## {i} ### {s.readline()} ######")
                    # time.sleep(0.35)
                    break

    s.close() 
    print()
    print(f"no of lines printed {count_ok}")
